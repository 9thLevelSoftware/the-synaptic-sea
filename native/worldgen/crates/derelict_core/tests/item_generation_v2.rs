...
fn loot_richness_is_monotonic_and_preserves_prefix() {
    ...
    assert_eq!(&low.items[..low.items.len()], &medium.items[..low.items.len()]);
    assert_eq!(&medium.items[..medium.items.len()], &high.items[..medium.items.len()]);
...
fn generated_output_obeys_budget_economy_and_trace_bounds() {
    ...
    assert!(outcome.drops.iter().all(|drop| drop.frequency_bp <= catalogue.caps.max_drop_frequency_bp));
}
...
fn generated_items_are_typed_compatible_with_authored_catalogue() {
    ...
    for item in &outcome.items {
        let family = catalogue.families.iter().find(|family| family.id == item.family_id).unwrap();
        let socket = catalogue.sockets.iter().find(|socket| socket.id == item.socket_id).unwrap();
        let rarity = catalogue.rarities.iter().find(|rarity| rarity.id == item.rarity_id).unwrap();
        ...
        for roll in &item.affixes {
            let definition = catalogue.affixes.iter().find(|affix| affix.id == roll.affix_id).unwrap();
...
fn invalid_context_and_malformed_authored_data_fail_closed() {
    ...
    duplicate_source_context.eligible_sources[1].source_id = duplicate_source_context.eligible_sources[0].source_id.clone();
...
fn drop_bindings_are_unique_and_match_items() {
    ...
    let source_ids: std::collections::BTreeSet<_> = outcome.drops.iter().map(|drop| &drop.source_id).collect();
