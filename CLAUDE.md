...
- **Project status source of truth:** `/STATUS.md` → `docs/game/inventory/SYSTEM_INVENTORY.md` + `docs/game/inventory/system_map.html` (canonical, code-verified inventory generated by `tools/build_system_inventory.py`; the `--check` smoke guards against drift) + `docs/game/integration_debt.md`. The older `docs/game/system_completion_audit.md` is **superseded** by the inventory (kept for narrative history). The old Gate 0–5 / "all-validated" roadmaps (`08_milestone_gates.md`, `09_system_roadmap.md`, `PLANNING_SYNTHESIS.md`, `build-plan.md`, `PROJECT_WORKSPACE.md`) were quarantined to `docs/archive/` on 2026-06-28 as inaccurate — do not cite them.
