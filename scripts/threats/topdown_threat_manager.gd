...
var threats: Array[ThreatAIStateScript] = []
...
func tick_threats(delta: float, player_position: Vector2, context: Dictionary = {}) -> void:
	for threat_state: ThreatAIStateScript in threats:
		...
		var old_state: String = str(threat_state.state)
...
func _move_threat(threat_state: ThreatAIStateScript, delta: float, player_pos: Vector2) -> void:
	var speed: float = float(threat_state.effective_move_speed())
	if speed <= 0.0:
		return

	var threat_pos: Vector2 = _threat_world_pos(threat_state)
	...
	if direction.length_squared() > 0.0:
		var movement: Vector2 = direction * speed * float(GridCoordinateScript.TILE_SIZE) * delta
		var new_pos: Vector2 = threat_pos + movement
