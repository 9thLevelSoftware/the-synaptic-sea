...
static func from_result(result: Variant):
	var state = load("res://scripts/ui/generated_world_prompt_state.gd").new()
	if typeof(result) != TYPE_OBJECT or not is_instance_valid(result) or not (result is LoadResult) or not result.validate(): return state
