...
const BASE_PANEL_SIZE: Vector2 = Vector2(300.0, 220.0)
...
func _ready() -> void:
	...
	label.custom_minimum_size = Vector2(260, 180)
...
func apply_accessibility_settings(settings: RefCounted) -> void:
	...
	size = size_scaled
	position = Vector2(-(size_scaled.x + 18.0), 18.0)
	label.custom_minimum_size = settings.scaled_hud_minimum_size(Vector2(260, 180))
	label.add_theme_font_size_override("font_size", settings.scaled_hud_font_size(BASE_FONT_SIZE))

func set_map_text(text: String) -> void:
	if label != null:
		label.text = text
