...
## Completing weld_patch on playable emits weld XP with away_from_start true.
## Marker: WORK WELD XP LIVE AWAY PASS away=true start=true complete=true xp=true
...
func _validate() -> void:
	...
	if bus.has_method("clear_log"):
		bus.clear_log()
	playable.module_integrity_map.ensure_module("eng/wall_weld_xp_away", "wall")
	var m = playable.module_integrity_map.get_module("eng/wall_weld_xp_away")
	m.integrity = 0.5
	m._recompute_state()
	...
	if not playable.work_action_driver.start_action("weld_patch", "eng/wall_weld_xp_away", ctx):
	...
	playable.emit_training_event(xp_ev, "eng/wall_weld_xp_away")
	var found := false
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			...
			if eid in ["weld_panel", "weld"]:
				...
				break
	if not found:
		_fail("weld_panel xp not logged event=%s away" % xp_ev); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("WORK WELD XP LIVE AWAY PASS away=true start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK WELD XP LIVE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
