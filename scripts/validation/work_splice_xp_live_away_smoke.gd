...
## Marker: WORK SPLICE XP LIVE AWAY PASS start=true complete=true xp=true
...
func _validate() -> void:
	...
	playable.module_integrity_map.ensure_module("eng/conduit_splice_away", "wall")
	...
	var res: Dictionary = playable.run_work_action_for_validation(
		"splice_conduit", "eng/conduit_splice_away", playable._inventory_qty_dict_for_work()
	)
	...
	print("WORK SPLICE XP LIVE AWAY PASS away=true start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK SPLICE XP LIVE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
