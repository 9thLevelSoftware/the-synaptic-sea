...
## Beginning a weapon reload routes SFX_TOOL_USE.
## Marker: WEAPON RELOAD SFX PASS reload=true sfx=true
...
func _validate() -> void:
	...
	# Equip a ranged weapon with ammo if needed.
	...
	# Equip into primary if possible
	if playable.has_method("_equip_from_inventory"):
		playable._equip_from_inventory(weapon_id, false)
	# Empty magazine so reload is needed
	if playable.ammo_state.has_method("configure"):
		pass
	...
	if after <= before:
		# Force: set mag empty via summary if begin failed
	...
	if after <= before:
		_fail("SFX_TOOL_USE not routed before=%d after=%d weapon=%s" % [before, after, weapon_id]); return
	print("WEAPON RELOAD SFX PASS reload=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WEAPON RELOAD SFX FAIL: %s" % msg)
	finished = true
	quit(1)
