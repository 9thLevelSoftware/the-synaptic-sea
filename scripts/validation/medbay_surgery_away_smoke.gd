...
## Medbay surgery success works with away_from_start true.
## Marker: MEDBAY SURGERY AWAY PASS away=true surgery=true heal=true xp=true
...
func _validate() -> void:
	...
	if not playable.try_medbay_surgery(playable.player):
		_fail("surgery should succeed away"); return
	if float(playable.vitals_state.health) <= 20.0:
		_fail("expected heal away"); return
	var found := false
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			if typeof(entry) == TYPE_DICTIONARY and str((entry as Dictionary).get("event_id", "")) == "perform_surgery":
				...
				break
	if not found:
		_fail("surgery XP missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("MEDBAY SURGERY AWAY PASS away=true surgery=true heal=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("MEDBAY SURGERY AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
