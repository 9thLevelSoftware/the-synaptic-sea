...
## In-range ToolPickup when already owned routes deny SFX and leaves the pile.
## Marker: TOOL PICKUP DENIED SFX PASS deny=true sfx=true remain=true
...
func _validate() -> void:
	...
	# Own the tool already so add_tool fails.
	...
	# Place player on the pickup so is_interact_candidate is true.
	...
	if not handled:
		_fail("interact not consumed on already-owned deny"); return
	...
	if bool(pickup.acquired):
		_fail("pickup marked acquired on deny"); return

	print("TOOL PICKUP DENIED SFX PASS deny=true sfx=true remain=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("TOOL PICKUP DENIED SFX FAIL: %s" % msg)
	finished = true
	quit(1)
