...
## Hallucination render routes SFX_SANITY_PHANTOM / HUD / AMBIENT on forced pool events.
## Marker: SANITY HALLUCINATION SFX PASS phantom=true hud=true ambient=true
...
func _validate() -> void:
	...
	if p_id < 0 or h_id < 0 or a_id < 0:
		_fail("force_trigger failed p=%d h=%d a=%d" % [p_id, h_id, a_id]); return
	...
	if p_after <= p_before:
		_fail("phantom sfx before=%d after=%d" % [p_before, p_after]); return
	if h_after <= h_before:
		_fail("hud sfx before=%d after=%d" % [h_before, h_after]); return
	if a_after <= a_before:
		_fail("ambient sfx before=%d after=%d" % [a_before, a_after]); return
	print("SANITY HALLUCINATION SFX PASS phantom=true hud=true ambient=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("SANITY HALLUCINATION SFX FAIL: %s" % msg)
	finished = true
	quit(1)
