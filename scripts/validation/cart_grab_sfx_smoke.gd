...
## Grabbing a salvage cart routes SFX_TOOL_PICKUP.
## Marker: CART GRAB SFX PASS grab=true sfx=true
...
func _validate() -> void:
	...
	if playable.grabbed_cart == null:
		_fail("grab failed cart=%s" % cart_id); return
	var after: int = int(mgr.sfx_router.get_routed_count(AudioEventSeamScript.SFX_TOOL_PICKUP))
	if after <= before:
		_fail("SFX_TOOL_PICKUP not routed before=%d after=%d" % [before, after]); return
	print("CART GRAB SFX PASS grab=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("CART GRAB SFX FAIL: %s" % msg)
	finished = true
	quit(1)
