...
##
## Marker: UNLOCK TRIGGER STREAM E AWAY PASS ration=true diagnose=true discover=true
...
func _validate() -> void:
	...
	print("UNLOCK TRIGGER STREAM E AWAY PASS away=true ration=true diagnose=true discover=true extract=true compound=true junk_salvage=true")
	_cleanup(0)
...
func _fail(reason: String) -> void:
	push_error("UNLOCK TRIGGER STREAM E AWAY FAIL reason=%s" % reason)
	finished = true
	_cleanup(1)
