...
## Empty cargo transfer deny SFX works with away_from_start true.
## Marker: CARGO EMPTY TRANSFER AWAY PASS away=true deposit=true withdraw=true panel=true
...
func _validate() -> void:
	...
	if moved != 0:
		_fail("expected empty deposit moved=0 away got %d" % moved); return
	if d_after <= d_before:
		_fail("empty deposit deny sfx missing away"); return
	...
	if withdrawn != 0:
		_fail("expected empty withdraw moved=0 away got %d" % withdrawn); return
	if w_after <= w_before:
		_fail("empty withdraw deny sfx missing away"); return
	...
	if bulk != 0:
		_fail("panel deposit_all expected 0 away got %d" % bulk); return
	if p_after <= p_before:
		_fail("panel empty deposit deny sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("CARGO EMPTY TRANSFER AWAY PASS away=true deposit=true withdraw=true panel=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("CARGO EMPTY TRANSFER AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
