...
## Medbay surgery refuse (health above threshold) routes UI_PANEL_CLOSE.
## Marker: MEDBAY SURGERY DENIED SFX PASS deny=true sfx=true
...
func _validate() -> void:
	...
	if ok:
		_fail("surgery should refuse at full health"); return
	if after <= before:
		_fail("UI_PANEL_CLOSE not routed before=%d after=%d" % [before, after]); return
	print("MEDBAY SURGERY DENIED SFX PASS deny=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("MEDBAY SURGERY DENIED SFX FAIL: %s" % msg)
	finished = true
	quit(1)
