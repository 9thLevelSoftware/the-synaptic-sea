...
## Transfer quantity denied SFX works with away_from_start true.
## Marker: TRANSFER QUANTITY DENIED AWAY PASS away=true deny=true sfx=true
...
func _validate() -> void:
	...
	if moved != 0:
		_fail("expected moved=0 away got %d" % moved); return
	if after <= before:
		_fail("deny sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("TRANSFER QUANTITY DENIED AWAY PASS away=true deny=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("TRANSFER QUANTITY DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
