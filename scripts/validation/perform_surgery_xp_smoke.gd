...
## perform_surgery training event emits (medbay surgery).
## Marker: PERFORM SURGERY XP PASS emit=true catalog=true
...
func _validate() -> void:
	...
	playable.emit_training_event("perform_surgery", "medbay")
	var found := false
	...
	print("PERFORM SURGERY XP PASS emit=true catalog=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("PERFORM SURGERY XP FAIL: %s" % msg)
	finished = true
	quit(1)
