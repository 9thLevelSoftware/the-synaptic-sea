...
## Junction calibrator apply routes SFX_TOOL_USE on successful consume.
## Marker: JUNCTION CALIBRATOR SFX PASS applied=true sfx=true
...
func _validate() -> void:
	...
	# Ensure a multi-step junction sequence and calibrator tool exist.
	...
	if after <= before:
		_fail("SFX_TOOL_USE not routed (calibrator may not have applied) before=%d after=%d" % [before, after]); return
	print("JUNCTION CALIBRATOR SFX PASS applied=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("JUNCTION CALIBRATOR SFX FAIL: %s" % msg)
	finished = true
	quit(1)
