...
## Inventory transfer with no selection routes UI_PANEL_CLOSE deny cue.
## Marker: TRANSFER DENIED SFX PASS deny=true sfx=true
...
func _validate() -> void:
	...
	if moved > 0:
		_fail("expected zero move with no selection"); return
	if after <= before:
		_fail("UI_PANEL_CLOSE not routed before=%d after=%d" % [before, after]); return
	print("TRANSFER DENIED SFX PASS deny=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("TRANSFER DENIED SFX FAIL: %s" % msg)
	finished = true
	quit(1)
