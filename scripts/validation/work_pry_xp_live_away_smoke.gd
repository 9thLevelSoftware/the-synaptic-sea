...
## Completing pry_panel on playable emits salvage XP with away_from_start true.
## Marker: WORK PRY XP LIVE AWAY PASS away=true start=true complete=true xp=true
...
func _validate() -> void:
	...
	if bus.has_method("clear_log"):
		bus.clear_log()
	playable.module_integrity_map.ensure_module("eng/wall_pry_xp_away", "wall")
	...
	var res: Dictionary = playable.run_work_action_for_validation(
		"pry_panel", "eng/wall_pry_xp_away", playable._inventory_qty_dict_for_work()
	)
	...
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			...
			if eid in ["salvage", "scavenge_container"]:
				...
				break
	if not found:
		_fail("salvage xp not logged away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("WORK PRY XP LIVE AWAY PASS away=true start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK PRY XP LIVE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
