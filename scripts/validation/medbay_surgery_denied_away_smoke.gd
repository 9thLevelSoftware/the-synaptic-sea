...
## Medbay surgery denied SFX works with away_from_start true.
## Marker: MEDBAY SURGERY DENIED AWAY PASS away=true deny=true sfx=true
...
func _validate() -> void:
	...
	if ok:
		_fail("surgery should refuse at full health away"); return
	if after <= before:
		_fail("deny sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("MEDBAY SURGERY DENIED AWAY PASS away=true deny=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("MEDBAY SURGERY DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
