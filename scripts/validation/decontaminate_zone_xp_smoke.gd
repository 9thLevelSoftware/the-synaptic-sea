...
## Marker: DECONTAMINATE ZONE XP PASS emit=true catalog=true
...
func _validate() -> void:
	...
	if text.find("decontaminate_zone") < 0:
		_fail("catalog"); return
	playable._on_fire_extinguished("cargo_01")
	var found := false
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			if typeof(entry) == TYPE_DICTIONARY and str((entry as Dictionary).get("event_id", "")) == "decontaminate_zone":
				...
				break
	if not found:
		_fail("decontaminate_zone not logged"); return
	print("DECONTAMINATE ZONE XP PASS emit=true catalog=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("DECONTAMINATE ZONE XP FAIL: %s" % msg)
	finished = true
	quit(1)
