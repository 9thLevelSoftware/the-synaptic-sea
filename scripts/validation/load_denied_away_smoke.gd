...
## Load denied SFX works with away_from_start true.
## Marker: LOAD DENIED AWAY PASS away=true deny=true sfx=true
...
func _validate() -> void:
	...
	if ok:
		_fail("load should fail without save away"); return
	if after <= before:
		_fail("deny sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("LOAD DENIED AWAY PASS away=true deny=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("LOAD DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
