...
## Hazard blocked SFX works with away_from_start true.
## Marker: HAZARD BLOCKED AWAY PASS away=true repair=true extinguish=true seal=true
...
func _validate() -> void:
	...
	if r1 <= r0:
		_fail("repair blocked SFX missing away"); return
	mgr.sfx_router.configure({})
	...
	if e1 <= e0:
		_fail("extinguish blocked SFX missing away"); return
	mgr.sfx_router.configure({})
	...
	if s1 <= s0:
		_fail("seal blocked SFX missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("HAZARD BLOCKED AWAY PASS away=true repair=true extinguish=true seal=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("HAZARD BLOCKED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
