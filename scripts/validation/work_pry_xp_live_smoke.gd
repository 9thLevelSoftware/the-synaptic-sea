...
## Completing pry_panel on playable emits salvage training XP.
## Marker: WORK PRY XP LIVE PASS start=true complete=true xp=true
...
func _validate() -> void:
	...
	playable.module_integrity_map.ensure_module("eng/wall_pry_xp", "wall")
	...
	var res: Dictionary = playable.run_work_action_for_validation(
		"pry_panel", "eng/wall_pry_xp", playable._inventory_qty_dict_for_work()
	)
	...
	print("WORK PRY XP LIVE PASS start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK PRY XP LIVE FAIL: %s" % msg)
	finished = true
	quit(1)
