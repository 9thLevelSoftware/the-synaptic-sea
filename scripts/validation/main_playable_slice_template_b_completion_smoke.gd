...
const PLAYABLE_SHIP_B: PackedScene = preload("res://scenes/procgen/playable_coherent_ship_002.tscn")
...
func _initialize() -> void:
	...
	if main_node == null:
		...
		return
	main_node.playable_ship_scene = PLAYABLE_SHIP_B
...
func _on_physics_frame() -> void:
	...
	if not hud_text.contains("Current: COMPLETE"):
		...
		return
	finished = true
	print("MAIN PLAYABLE TEMPLATE B COMPLETE PASS completed=%d current_sequence=%d run_complete=true" % [completed, current_sequence])
	quit(0)
...
func _fail(reason: String) -> void:
	if finished:
		return
	finished = true
	push_error("MAIN PLAYABLE TEMPLATE B COMPLETE FAIL reason=%s" % reason)
	quit(1)
