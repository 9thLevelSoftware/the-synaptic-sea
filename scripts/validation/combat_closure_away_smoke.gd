...
##
## Pass marker:
##   COMBAT CLOSURE AWAY PASS away_kill=true noise=true crouch=true reward=true removed=true pending_corpse=true
...
func _validate() -> void:
	...
	print("COMBAT CLOSURE AWAY PASS away=true away_kill=true noise=true crouch=true reward=true removed=true pending_corpse=true coord_feed=true")
	_cleanup_and_quit(0)
...
func _fail(reason: String) -> void:
	if finished:
		return
	finished = true
	push_error("COMBAT CLOSURE AWAY FAIL reason=%s" % reason)
	_cleanup_and_quit(1)
