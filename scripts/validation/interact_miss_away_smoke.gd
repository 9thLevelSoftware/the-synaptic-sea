...
## Interact miss SFX works with away_from_start true.
## Marker: INTERACT MISS AWAY PASS away=true miss=true sfx=true
...
func _validate() -> void:
	...
	if after <= before:
		_fail("miss sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("INTERACT MISS AWAY PASS away=true miss=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("INTERACT MISS AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
