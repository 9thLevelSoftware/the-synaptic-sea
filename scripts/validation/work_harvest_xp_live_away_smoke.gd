...
## Completing harvest_crop WorkAction emits cooking training XP.
## Marker: WORK HARVEST XP LIVE AWAY PASS start=true complete=true xp=true
...
func _validate() -> void:
	...
	var res: Dictionary = playable.run_work_action_for_validation(
		"harvest_crop", "hydro_bed_0", playable._inventory_qty_dict_for_work()
	)
	if not bool(res.get("ok", false)):
		...
		if not playable.work_action_driver.start_action("harvest_crop", "hydro_bed_0", ctx):
			_fail("start harvest %s" % str(res)); return
	...
	print("WORK HARVEST XP LIVE AWAY PASS away=true start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK HARVEST XP LIVE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
