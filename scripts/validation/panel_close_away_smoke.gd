...
## Panel close SFX works with away_from_start true.
## Marker: PANEL CLOSE AWAY PASS away=true wounds=true shipmod=true chart=true scanner=true recipe=true
...
func _validate() -> void:
	...
	for pair in checks:
		...
		if after <= before:
			_fail("%s close sfx missing away" % key); return
		flags[key] = true
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("PANEL CLOSE AWAY PASS away=true wounds=true shipmod=true chart=true scanner=true recipe=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("PANEL CLOSE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
