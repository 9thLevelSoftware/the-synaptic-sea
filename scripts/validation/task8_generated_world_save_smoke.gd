...
func _init() -> void:
	...
	var envelope := Envelope.new(); assert(envelope.configure(7, 3, "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", MAP, [{"identity":identity.to_dict(), "mutation_delta":delta.to_dict()}]))
