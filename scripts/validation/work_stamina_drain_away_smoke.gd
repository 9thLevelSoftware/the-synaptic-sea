...
## Work stamina drain works with away_from_start true.
## Marker: WORK STAMINA DRAIN AWAY PASS away=true start=true drain=true speed=true
...
func _validate() -> void:
	...
	playable.module_integrity_map.ensure_module("eng/wall_drain_away", "wall")
	...
	if not playable.work_action_driver.start_action("cut_wall", "eng/wall_drain_away", ctx):
		_fail("start_action cut_wall failed away"); return
	...
	if after >= before:
		_fail("expected stamina drain away before=%s after=%s" % [str(before), str(after)]); return
	if after > before - 4.0:
		_fail("expected ~8 stamina/s drain away got delta=%s" % str(before - after)); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("WORK STAMINA DRAIN AWAY PASS away=true start=true drain=true speed=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK STAMINA DRAIN AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
