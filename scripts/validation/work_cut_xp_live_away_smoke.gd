...
## Marker: WORK CUT XP LIVE AWAY PASS start=true complete=true xp=true
...
func _validate() -> void:
	...
	playable.module_integrity_map.ensure_module("eng/wall_xp_away", "wall")
	...
	var res: Dictionary = playable.run_work_action_for_validation(
		"cut_wall", "eng/wall_xp_away", playable._inventory_qty_dict_for_work()
	)
	if not bool(res.get("ok", false)):
		...
		if not playable.work_action_driver.start_action("cut_wall", "eng/wall_xp_away", ctx):
		...
		if not xp_ev.is_empty():
			playable.emit_training_event(xp_ev, "eng/wall_xp_away")
	var found := false
	...
	print("WORK CUT XP LIVE AWAY PASS away=true start=true complete=true xp=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK CUT XP LIVE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
