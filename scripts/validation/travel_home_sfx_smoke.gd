...
## travel_home success routes SFX_DOCK_LAND.
## Marker: TRAVEL HOME SFX PASS home=true sfx=true
...
func _validate() -> void:
	...
	if playable.audio_manager == null or playable.audio_manager.sfx_router == null:
		_fail("audio"); return
	# Force away state so travel_home can run without full travel setup.
	...
	if after <= before:
		_fail("SFX_DOCK_LAND not routed before=%d after=%d" % [before, after]); return
	print("TRAVEL HOME SFX PASS home=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("TRAVEL HOME SFX FAIL: %s" % msg)
	finished = true
	quit(1)
