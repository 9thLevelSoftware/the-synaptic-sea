...
## Bandaging a wound consumes bandage_kit from InventoryState.
## Marker: WOUND BANDAGE INVENTORY PASS wound=true bandage=true consume=true
...
func _validate() -> void:
	...
	# Without bandage, fail
	if bool(playable.bandage_wound_with_inventory_for_validation().get("ok", true)):
		# may fail no wound left or no item - add another wound but no kit
		playable.wound_state.apply_wound({
			"kind": "puncture",
			"body_part": "leg",
			"severity": 0.4,
	...
	print("WOUND BANDAGE INVENTORY PASS wound=true bandage=true consume=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WOUND BANDAGE INVENTORY FAIL: %s" % msg)
	finished = true
	quit(1)
