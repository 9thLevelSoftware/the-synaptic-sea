...
## Ship-mod install/uninstall emit construction training XP events.
## Marker: SHIP MOD INSTALL XP PASS install_xp=true uninstall_xp=true catalog=true
...
func _validate() -> void:
	...
	if text.find("ship_mod_install") < 0 or text.find("ship_mod_uninstall") < 0:
	...
	if not found_install:
		_fail("ship_mod_install not in training log"); return
	panel.refresh()
	...
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			if typeof(entry) == TYPE_DICTIONARY and str((entry as Dictionary).get("event_id", "")) == "ship_mod_uninstall":
				...
				break
	if not found_un:
		_fail("ship_mod_uninstall not in training log"); return
	print("SHIP MOD INSTALL XP PASS install_xp=true uninstall_xp=true catalog=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("SHIP MOD INSTALL XP FAIL: %s" % msg)
	finished = true
	quit(1)
