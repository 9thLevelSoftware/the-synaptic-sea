...
## Marker: AMMO MAGAZINE AWAY PASS away_ticks=<n> spent=true dry_fire=true reloaded=true
...
func _validate() -> void:
	...
	if spent and dry_fire and reloaded:
		print("AMMO MAGAZINE AWAY PASS away_ticks=%d spent=true dry_fire=true reloaded=true" % n)
		_cleanup(0)
	else:
...
func _fail(reason: String) -> void:
	push_error("AMMO MAGAZINE AWAY FAIL reason=%s" % reason)
	_cleanup(1)
