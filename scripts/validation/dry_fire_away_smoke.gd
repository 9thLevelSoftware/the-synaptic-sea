...
## Empty-magazine dry-fire SFX works with away_from_start true.
## Marker: DRY FIRE AWAY PASS away=true dry=true sfx=true
...
func _validate() -> void:
	...
	if after <= before:
		_fail("dry fire sfx missing away res=%s" % str(res)); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("DRY FIRE AWAY PASS away=true dry=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("DRY FIRE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
