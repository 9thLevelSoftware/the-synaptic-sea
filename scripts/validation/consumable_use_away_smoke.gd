...
## Consumable use fail/success SFX works with away_from_start true.
## Marker: CONSUMABLE USE AWAY PASS away=true ok=true fail=true
...
func _validate() -> void:
	...
	if bool(bad.get("ok", false)):
		_fail("expected fail use away"); return
	if f1 <= f0:
	...
	if not bool(good.get("ok", false)):
		_fail("expected successful consumable use away"); return
	if s1 <= s0:
		_fail("success SFX missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return

	print("CONSUMABLE USE AWAY PASS away=true ok=true fail=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("CONSUMABLE USE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
