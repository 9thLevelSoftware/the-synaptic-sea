...
## Repair start emits diagnose_fault training XP.
## Marker: DIAGNOSE FAULT XP PASS emit=true catalog=true
	...
	if bus.has_method("get_log"):
		for entry in bus.get_log():
			if typeof(entry) == TYPE_DICTIONARY and str((entry as Dictionary).get("event_id", "")) == "diagnose_fault":
				...
				break
	if not found:
		_fail("diagnose_fault not logged"); return
	print("DIAGNOSE FAULT XP PASS emit=true catalog=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("DIAGNOSE FAULT XP FAIL: %s" % msg)
	finished = true
	quit(1)
