...
## Work-yield stack-full scoop deny works with away_from_start true.
## Marker: WORK YIELD SCOOP DENIED AWAY PASS away=true deny=true sfx=true remain=true
...
func _validate() -> void:
	...
	playable.inventory_state.items["scrap_metal"] = 20
	...
	if not handled:
		_fail("interact not consumed away"); return
	var after: int = int(mgr.sfx_router.get_routed_count(AudioEventSeamScript.UI_PANEL_CLOSE))
	if after <= before:
		_fail("deny sfx missing away"); return
	if not is_instance_valid(drop) or int(drop.items.get("scrap_metal", 0)) != 2:
		_fail("drop corrupted away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return

	print("WORK YIELD SCOOP DENIED AWAY PASS away=true deny=true sfx=true remain=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK YIELD SCOOP DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
