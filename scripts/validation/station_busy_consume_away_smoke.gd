...
## Station busy consume works with away_from_start true.
## Marker: STATION BUSY CONSUME AWAY PASS away=true craft=true hydro=true
...
func _validate() -> void:
	...
	if not st.try_interact(playable.player):
		_fail("craft busy did not consume interact away"); return
	...
	if not hydro.try_interact(playable.player):
		_fail("hydro in_progress did not consume interact away"); return

	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("STATION BUSY CONSUME AWAY PASS away=true craft=true hydro=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("STATION BUSY CONSUME AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
