...
## Footstep SFX works with away_from_start true.
## Marker: FOOTSTEP AWAY PASS away=true moving=true sfx=true
...
func _validate() -> void:
	...
	if after <= before:
		_fail("footstep sfx missing away"); return
	...
	if idle_after != after:
		_fail("footstep fired while idle away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("FOOTSTEP AWAY PASS away=true moving=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("FOOTSTEP AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
