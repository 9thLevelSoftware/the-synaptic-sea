...
## Pause menu close SFX works with away_from_start true.
## Marker: PAUSE MENU CLOSE AWAY PASS away=true close=true sfx=true
...
func _validate() -> void:
	...
	playable.menu_coordinator.menu_state.open_menu("pause_menu")
	...
	playable.menu_coordinator._emit_menu_close_sfx()
	var after: int = int(mgr.sfx_router.get_routed_count(AudioEventSeamScript.UI_PANEL_CLOSE))
	if after <= before:
		_fail("close sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("PAUSE MENU CLOSE AWAY PASS away=true close=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("PAUSE MENU CLOSE AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
