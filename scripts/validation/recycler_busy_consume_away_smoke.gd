...
## Recycler busy consume works with away_from_start true.
## Marker: RECYCLER BUSY CONSUME AWAY PASS away=true recycling=true consume=true
...
func _validate() -> void:
	...
	if not st.try_interact(playable.player):
		_fail("recycler recycling did not consume interact away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("RECYCLER BUSY CONSUME AWAY PASS away=true recycling=true consume=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("RECYCLER BUSY CONSUME AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
