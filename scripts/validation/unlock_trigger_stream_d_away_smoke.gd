...
##
## Marker: UNLOCK TRIGGER STREAM D AWAY PASS scan=true first_aid=true cook=true
...
func _validate() -> void:
	...
	print("UNLOCK TRIGGER STREAM D AWAY PASS away=true scan=true first_aid=true cook=true fabricate=true repair=true weld=true travel=true")
	_cleanup(0)
...
func _fail(reason: String) -> void:
	push_error("UNLOCK TRIGGER STREAM D AWAY FAIL reason=%s" % reason)
	finished = true
	_cleanup(1)
