...
## Recycler no-input consume works with away_from_start true.
## Marker: RECYCLER NO INPUT CONSUME AWAY PASS away=true no_input=true consume=true
...
func _validate() -> void:
	...
	if not st.try_interact(playable.player):
		_fail("no_input did not consume interact away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("RECYCLER NO INPUT CONSUME AWAY PASS away=true no_input=true consume=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("RECYCLER NO INPUT CONSUME AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
