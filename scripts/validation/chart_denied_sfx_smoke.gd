...
## Opening chart without web_chart item routes UI_PANEL_CLOSE deny cue.
## Marker: CHART DENIED SFX PASS denied=true sfx=true
...
func _validate() -> void:
	...
	if opened:
		_fail("chart should refuse without web_chart"); return
	if after <= before:
		_fail("UI_PANEL_CLOSE not routed before=%d after=%d" % [before, after]); return
	print("CHART DENIED SFX PASS denied=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("CHART DENIED SFX FAIL: %s" % msg)
	finished = true
	quit(1)
