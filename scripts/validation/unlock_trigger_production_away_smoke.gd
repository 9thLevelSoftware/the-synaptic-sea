...
##
## Pass marker: UNLOCK TRIGGER PRODUCTION AWAY PASS triggers_valid=true scavenge_emitted=true codex_unlocked=true class_unlocked=true bridge_unlocked=true
...
func _validate() -> void:
	...
	print("UNLOCK TRIGGER PRODUCTION AWAY PASS away=true triggers_valid=true scavenge_emitted=true codex_unlocked=true class_unlocked=true bridge_unlocked=true")
	_cleanup(0)
...
func _fail(reason: String) -> void:
	push_error("UNLOCK TRIGGER PRODUCTION AWAY FAIL reason=%s" % reason)
	finished = true
	_cleanup(1)
