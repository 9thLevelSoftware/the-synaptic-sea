...
## Save denied SFX works with away_from_start true.
## Marker: SAVE DENIED AWAY PASS away=true denied=true sfx=true
...
func _validate() -> void:
	...
	if ok:
		_fail("save should refuse when slice_complete away"); return
	if after <= before:
		_fail("deny sfx missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("SAVE DENIED AWAY PASS away=true denied=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("SAVE DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
