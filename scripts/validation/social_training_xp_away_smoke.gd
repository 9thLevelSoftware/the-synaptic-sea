...
## Social training XP works with away_from_start true.
## Marker: SOCIAL TRAINING XP AWAY PASS away=true inspire=true negotiate=true intimidate=true
...
func _validate() -> void:
	...
	for eid in ["inspire_crew", "negotiate_truce", "intimidate_threat", "transmit_relay"]:
		...
		playable.emit_training_event(eid, "test_target_away")
	...
	print("SOCIAL TRAINING XP AWAY PASS away=true inspire=true negotiate=true intimidate=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("SOCIAL TRAINING XP AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
