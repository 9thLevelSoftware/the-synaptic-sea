...
## Work interrupt on damage routes UI_PANEL_CLOSE cancel cue.
## Marker: WORK INTERRUPT SFX PASS interrupt=true sfx=true
...
func _validate() -> void:
	...
	if playable.module_integrity_map != null:
		playable.module_integrity_map.ensure_module("eng/wall_int_sfx", "wall_straight_1x1", {"scrap_metal": 2}, "eng")
	var inv: Dictionary = {"welding_lance": 1}
	if playable.inventory_state != null:
		playable.inventory_state.add_item("welding_lance", 1)
		inv = playable.inventory_state.get_summary().get("items", inv) if playable.inventory_state.has_method("get_summary") else inv
	var ctx: Dictionary = {
	...
	if not playable.work_action_driver.start_action("cut_wall", "eng/wall_int_sfx", ctx):
		_fail("start_action failed"); return
	...
	print("WORK INTERRUPT SFX PASS interrupt=true sfx=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("WORK INTERRUPT SFX FAIL: %s" % msg)
	finished = true
	quit(1)
