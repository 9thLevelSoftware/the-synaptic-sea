...
## cook_meal training event emits (kitchen / hydroponics).
## Marker: COOK MEAL XP PASS emit=true catalog=true
...
func _validate() -> void:
	...
	playable.emit_training_event("cook_meal", "stew")
	var found := false
	...
	print("COOK MEAL XP PASS emit=true catalog=true")
	quit(0)
