...
## REQ-WA-002: structure WorkAction yields land in InventoryState under away_from_start.
## Marker: WORK YIELD INVENTORY AWAY PASS away=true cut=true scrap=true qty=true
...
func _start() -> void:
	...
	if str(playable.work_action_driver.work.get("action_id")) != "cut_wall":
		var mid: String = "hub/yield_wall_away"
		...
		print("WORK YIELD INVENTORY AWAY PASS away=true cut=true scrap=true qty=true")
		finished = true
		quit(0)
		return
	phase = "tick"
...
func _fail(msg: String) -> void:
	print("WORK YIELD INVENTORY AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
