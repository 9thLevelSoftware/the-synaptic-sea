...
## Quicksave deny (and success) SFX works with away_from_start true.
## Marker: QUICKSAVE DENIED AWAY PASS away=true deny=true save=true
...
func _validate() -> void:
	...
	if not ok:
		_fail("first quicksave failed away"); return
	if s1 <= s0:
		_fail("UI_SAVE not routed away"); return
	...
	if not denied:
		_fail("second quicksave should be cooldown-denied away"); return
	if d1 <= d0:
		_fail("deny SFX missing away"); return
	if not bool(playable.away_from_start):
		_fail("away cleared"); return
	print("QUICKSAVE DENIED AWAY PASS away=true deny=true save=true")
	quit(0)
...
func _fail(msg: String) -> void:
	print("QUICKSAVE DENIED AWAY FAIL: %s" % msg)
	finished = true
	quit(1)
