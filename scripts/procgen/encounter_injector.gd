...
func inject(
	...
	# Combined density multiplier for the encounter dial. Floor at 0 but DO NOT cap at
	# 1.0 here — biome/difficulty density > 1.0 (breach_field 1.3, deep_dive 1.6, their
	# 2.08 combination) must be able to RAISE the spawn rate, not just lower it. The
	...
	if biome != null and difficulty != null:
		density = maxf(_safe_combined(biome, difficulty, DIAL_ENCOUNTER), 0.0)
	elif difficulty != null:
		density = maxf(_safe_modifier(difficulty, DIAL_ENCOUNTER), 0.0)
