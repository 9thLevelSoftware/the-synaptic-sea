...
func save(state: SettingsState) -> bool:
	...
	if document.to_utf8_buffer().size() > MAX_BYTES: f.close(); DirAccess.remove_absolute(ProjectSettings.globalize_path(path)); return false
	...
	if had_final and DirAccess.rename_absolute(ProjectSettings.globalize_path(FILE_PATH), ProjectSettings.globalize_path(backup_path)) != OK: DirAccess.remove_absolute(ProjectSettings.globalize_path(path)); return false
	var result := DirAccess.rename_absolute(ProjectSettings.globalize_path(path), ProjectSettings.globalize_path(FILE_PATH))
	if result != OK:
		DirAccess.remove_absolute(ProjectSettings.globalize_path(path)); if had_final: DirAccess.rename_absolute(ProjectSettings.globalize_path(backup_path), ProjectSettings.globalize_path(FILE_PATH)); return false
...
func _valid_payload(payload: Variant) -> bool:
	...
	if typeof(payload.get("text_scale")) not in [TYPE_FLOAT, TYPE_INT] or float(payload.text_scale) < Schema.MIN_TEXT_SCALE or float(payload.text_scale) > Schema.MAX_TEXT_SCALE: return false
	...
	if typeof(payload.preset_id) != TYPE_STRING or payload.preset_id.is_empty(): return false
