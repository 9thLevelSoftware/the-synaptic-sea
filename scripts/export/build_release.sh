...
SARGASSO_VERSION=${SARGASSO_VERSION:-v0.1.0}
BUILD_STAMP=${SARGASSO_BUILD_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
...
named sargasso-of-stars-<version>-<stamp>-<target>.*.
USAGE
}
...
prepare_release_project() {
  PROJECT_BACKUP=$(mktemp "${TMPDIR:-/tmp}/sargasso_project_godot.XXXXXX")
...
main() {
  ...
  for target in "${targets[@]}"; do
    case "$target" in
      web)
        export_one web "$EXPORT_DIR/web/index.html" "$RELEASE_DIR/sargasso-of-stars-${SARGASSO_VERSION}-${BUILD_STAMP}-web.zip"
        ;;
      linux)
        export_one linux "$EXPORT_DIR/linux/sargasso-of-stars.x86_64" "$RELEASE_DIR/sargasso-of-stars-${SARGASSO_VERSION}-${BUILD_STAMP}-linux-x86_64.zip"
        ;;
      macos)
        export_one macos "$EXPORT_DIR/macos/sargasso-of-stars.zip" "$RELEASE_DIR/sargasso-of-stars-${SARGASSO_VERSION}-${BUILD_STAMP}-macos.zip"
        ;;
      windows)
        export_one windows "$EXPORT_DIR/windows/sargasso-of-stars.exe" "$RELEASE_DIR/sargasso-of-stars-${SARGASSO_VERSION}-${BUILD_STAMP}-windows-x86_64.zip"
        ;;
      *)
  ...
  echo "SARGASSO EXPORT PASS version=$SARGASSO_VERSION stamp=$BUILD_STAMP targets=${targets[*]} release_dir=$RELEASE_DIR"
}
