...
## Pre-polish program (started 2026-07-22; mechanical packages through #525)
...
**Landed (PRs #78–#525, 2026-07-22 → 2026-07-23):**
...
`SYNAPTIC_SEA REGRESSION PASS commands=627 clean_output=true` (marker-based; set `GODOT`/`ROOT` on Windows).

**Post-INT hardening (PRs #112–#525):**
...
- Ship-mod toggle works with away_from_start
- Wounds toggle works with away_from_start
- Attack soft-fail deny SFX on away branch
- Melee reload deny SFX on away branch
- Work-yield scoop deny SFX on away branch
- Hatch bypass deny SFX on away branch
- Work-yield partial scoop on away branch
- Loot empty-grant deny + loot search success SFX on away branch
- Tool pickup already-owned deny SFX on away branch
- Reload denied + interact miss SFX on away branch
- Dry-fire, consumable use, equip deny, work-yield scoop SFX on away branch
- Combat hit, unequip empty, field craft start, transfer deny on away branch
- Threat alert, encumbrance overload, weapon reload, player attack on away branch
- Work interrupt, room discover, extract data, footstep SFX on away branch
- Hatch open/close, tool pickup success, work-yield drop SFX on away branch
- Sanity hallucination render, field-craft block, wounds treat deny, equip/unequip on away branch
- Hazard blocked, cart grab/deny, transfer-quantity deny SFX on away branch
- Work start, field-craft begin-blocked, inventory transfer, end-run SFX on away branch
- Travel deny, scanner open, ship-mod install fail, empty cargo transfer on away branch
- Scanner confirm deny, recipe picker deny, chart route, wounds panel open on away branch
- Transfer panel open, recipe picker open, chart denied, dock barrier open on away branch
- Craft blocked/start/picker-deny + production blocked SFX on away branch
- Production start + save denied / load denied / autosave SFX on away branch
- Quicksave cooldown, cargo bulk transfer, hangar deny, travel home on away branch
- Medbay surgery deny, inspire crew, hull groan, achievement unlock on away branch
- Dock land, hangar dock/launch, bridge login deny, multi-panel close on away branch
- Meta biomatter/reactor, pause open, junction calibrator, medbay surgery on away branch
- Pause close, settings/records open, field craft complete SFX on away branch
- Settings/records back, resume confirm, work progress UI pulse on away branch
- Codex open, UI load, tutorial, repair complete, treat wound on away branch
- **Dual-branch SFX coverage complete:** every `*_sfx_smoke` has an away counterpart
- Consume-path away batch: hazard channel, station busy, repair soft-block, dock barrier channel
- Consume-path away batch 2: recycler busy/no-input, plant blocked, production output full
- Production unknown-kind consume on away branch
- Work hold-to-work freeze, zero-stamina start block, stamina interrupt on away branch
- Work stamina drain while ticking WorkAction on away branch
- Bandage/treat first_aid training XP on away branch
- Training XP away batch: decode_signal, breach seal, cook_meal, decontaminate_zone
- Training XP away batch 2: threat kill, salvage scavenge, diagnose_fault, fabricate_part
- Training XP away batch 3: surgery, harvest cook_meal, scan_derelict, ship_mod_install
- Training XP away batch 4: social suite, travel plot/astro, repair_subcomponent, consumable XP
- Training XP away batch 5 (#512): discover_room/extract_data, build_shelter, compound_stim, first_aid_ally
- Training XP away batch 6: repair_full_system, stream D/E ration/scan/transmit, ship_mod_uninstall, weld_panel/weld
- Training XP away batch 7: defeat_enemy + cooking/salvage/repair work aliases (catalog dual-branch complete)
- Work live XP dual-branch: pry/weld/cut/patch/splice/suppress/harvest/plant complete on away
- Component mount/remount live XP+SFX + work complete/suppress SFX live on away
- Component mount/dismount + work-action interact paths complete on away
- Weld damaged/skill, cut module scene, ship-mod install/inv/plating, component placement on away
- Ship-mod overbudget/panel/power budget/restore/station tier/system effect on away
- Work verb SFX multi/patch-splice/plant-harvest + work integration + plating catalog + repair channel on away
- Work yield inventory, production station wiring, wound bandage inventory on away
- Unlock trigger production + streams D/E/F, ship catchup, component markers on away
- Full training_actions catalog emit coverage under away_from_start
- Hazard feedback (extinguish/seal blocked + seal SFX) under away via active fire/hull seams
- Ammo magazine, flare steady, combat closure, derelict fire-zone marker on away
- Regression marker contract commands=627 in 06_validation_plan.md
...
**Next:** remaining production dual-branch / content polish; inventory `--check` remains green (`systems=191`).
