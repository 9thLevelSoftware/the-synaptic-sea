...
- Default assignee: `synapse_seaworker` (bulk execution).
- Implementation worker: `synapse_seaworker` on `MiniMax-M3`.
- Documentation/planning worker: `synapse_seadocs` on `kimi-k2.7-code` (OpenCode Go).
- Review/gate worker: `synapse_seareview` on `gpt-5.5` (OpenAI Codex).
