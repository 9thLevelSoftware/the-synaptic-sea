...
STAGED_PROPS_RELATIVE = Path("assets/_staging/focused_nine/props")
STAGED_ASSET_IDS = frozenset({"fire_suppression_station", "hull_breach_seal_point"})
STAGED_EXTENSIONS = {
...
def _location_diagnostics(
    ...
    stage_lexical = root / STAGED_PROPS_RELATIVE
    errors: list[str] = []
    ...
    try:
        ...
        return [f"focused-nine staging path could not be resolved: {exc}"], lexical, False

    expected_name = f"{asset_id}.glb" if asset_id else None
    if asset_id not in STAGED_ASSET_IDS:
        errors.append(
            "asset_id must be one of: fire_suppression_station, hull_breach_seal_point"
        )
    ...
    if lexical.parent != stage_lexical:
        errors.append(
            ...
            f"{STAGED_PROPS_RELATIVE.as_posix()}/<asset_id>.glb"
        )
    ...
    return _sorted_errors(errors), resolved, path_valid


def _expected_visual_scene_path(asset_id: str) -> str:
    return f"res://{STAGED_PROPS_RELATIVE.as_posix()}/{asset_id}.glb"
...
def build_staged_sidecar(project_root: Path, glb_path: Path, asset_id: str) -> dict:
    ...
    location_errors, resolved_glb, path_valid = _location_diagnostics(
    ...
    sidecar: dict[str, Any] = {
        ...
        "asset_id": asset_id,
        "prop_kind": "dressing",
        "visual_scene_path": _expected_visual_scene_path(asset_id),
    ...
    errors = validate_staged_sidecar(project_root, resolved_glb, sidecar)
    if errors:
...
def validate_staged_sidecar(project_root: Path, glb_path: Path, sidecar: dict) -> list[str]:
    ...
    if asset_id not in STAGED_ASSET_IDS:
        ...
        expected_path = _expected_visual_scene_path(asset_id)
