...
Parent spec: `docs/superpowers/specs/2026-06-20-sargasso-core-systems-design.md` (System 4 + System 8)
...
infinite Sargasso of ship markers; a scanner whose reach/detail is gated by the ship's
...
- **No `RunSnapshot` integration this phase.** Each model exposes `get_summary`/`apply_summary`
  (round-trip validated by smokes), but wiring the Sargasso world into a save belongs to a later
...
SargassoWorld (RefCounted)                scripts/systems/sargasso_world.gd
...
`distance` is derived against the player position by `SargassoWorld`, never stored.

### `MarkerGenerator` (deterministic, infinite)

```gdscript
const CELL_SIZE := 100.0          # Sargasso-space units per grid cell
...
position within the cell (x,z in world units; y=0 — Sargasso travel is planar), `seed_value` from
...
2. **`sargasso_world_smoke.gd`** — `markers_in_range(radius)` returns markers within radius sorted by
...
5. Register all four in `docs/game/06_validation_plan.md` (`commands=54` → `58`); full bundle must
   end `SARGASSO REGRESSION PASS ... clean_output=true`; Gate-1 playtest still passes.

## File structure

- Create: `scripts/systems/ship_marker.gd`, `scripts/systems/marker_generator.gd`,
  `scripts/systems/sargasso_world.gd`, `scripts/systems/scanner_state.gd`,
