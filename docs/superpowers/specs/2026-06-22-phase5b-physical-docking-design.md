...
| Player exactly on a dock seam | Deterministic tiebreak: the **piloted ship wins containment** (it is listed first in the occupancy entries). This *supersedes* the 5a "host wins seam" rule — in the ride-aboard model the player is bodily in their ship until they breach the dock barrier and cross fully into the host (the closed barrier is the boarding gate). Implementation note: with the generous `interior_aabb` (`ROOM_HALF_EXTENT=4.0`), the piloted ship's AABB overlaps the host's near rooms, so occupancy flips to the host only once the player is in host-only space past the overlap; tightening `interior_aabb` would make the flip crisper (tracked for the occupancy/boarding cycle). Recorded in ADR-0017. |
