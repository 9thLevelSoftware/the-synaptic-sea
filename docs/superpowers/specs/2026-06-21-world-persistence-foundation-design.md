...
Related: ADR-0007 (save/load scope), ADR-0011 (ShipInstance & travel integration), `docs/superpowers/specs/2026-06-20-sargasso-core-systems-design.md` (System 5/8)
...
- `scripts/systems/sargasso_world.gd` — `SargassoWorld`: `world_seed`, map-space
...
```
WorldSnapshot
├── world_summary             # SargassoWorld.get_summary()
...
- **First visit to a marker:** generate from seed, build a `ShipInstance`, register it in
  `visited_ships`, make it the active ship. (`SargassoWorld.mark_generated` still records the
  marker.)
...
Load rebuilds: apply `world_summary` to `SargassoWorld`, restore the `visited_ships`
...
Map space (`SargassoWorld.player_position`, in `world_summary`) and scene space
