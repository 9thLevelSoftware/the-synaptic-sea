...
- `Interactable` gains `tooltip_subject_id: String`, set where `prompt_text` is set today: `configure_from_objective` → `objective_type` (matches existing catalog subject ids), `configure_from_step` → `"junction_step"` (one new catalog entry covers repair steps generically).
