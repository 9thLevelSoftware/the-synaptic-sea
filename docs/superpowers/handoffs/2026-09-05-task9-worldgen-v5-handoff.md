...
- `73851214` — isolate real-dispatcher hazard smokes from higher-priority co-located affordances
- `65a13021` — align the survival speed gate with the continuous stamina curve
- `5b783998` — use a catalogued deferred-content sentinel for honest spatial-audio fallback
...
A subsequent non-authoritative resume process (`proc_26bba5c29fd8`) likewise advanced through main item economy, spoilage, breach-seal, and life-support gates, then exited after printing the `Domain 1 survival stakes (home) smoke` heading before Godot emitted any startup output. Parent reconciliation found no residual process and reproduced survival-stakes both alone and immediately after the life-support gate; both target runs exited 0 with `MAIN PLAYABLE SURVIVAL STAKES PASS gate_half=true gate_locked=true death=true reachable=true` and zero diagnostics. This is the same external launch-boundary termination pattern, not a source regression or cross-process state leak. The remaining resume rows were not run under the active stop.

A third non-authoritative resume process (`proc_0e20c73a298c`) passed every logged gate from survival-stakes through audio-content coverage, including the audio gates that emitted only the two explicitly allowlisted missing voice-log warnings, then exited after printing the `REQ-AU-005 spatial audio playback smoke` heading before Godot emitted startup output. Parent reconciliation found no residual OS process or active managed resume session and reproduced spatial-audio playback both alone and immediately after audio-content coverage; both target runs exited 0 with `AUDIO SPATIAL PASS catalogued_playing=true fallback_honest=true production_pickup=true position_tracked=true` and zero diagnostics. This is again external launch-boundary termination, not a product regression or state leak. No broader resume was started, and all managed resume sessions are stopped or exited under the active pause.

A fourth non-authoritative resume process (`proc_117a721baa2a`) passed spatial audio, systems-map currency, requirement trace, the explicitly accepted absent-board `KANBAN MANIFEST SKIP`, and system-inventory anti-drift, then exited after printing the `architecture diagram anti-drift check` heading before its npm/Python child emitted output. Parent reconciliation found no residual OS process or active managed resume session and ran the exact locked command (`npm --prefix tools/architecture ci --silent` followed by `validate_architecture_diagrams.py --check`) both alone and after system inventory; both target runs exited 0 with `ARCHITECTURE DIAGRAMS PASS diagrams=5 exports=5 references=49`, and the worktree remained clean. This is again external launch-boundary termination, not an architecture drift failure or suite state leak. No broader resume was started under the active pause.
...
Commit `73851214` (`test: isolate co-located hazard dispatch smokes`) follows `3501cbe9` with exactly two test-only files: `main_playable_item_economy_smoke.gd` and `main_playable_life_support_vitals_smoke.gd`. Both retain the real `_on_player_interact_requested()` dispatcher and channel-completion path, but before teleport/interaction they extinguish ambient fires and clear higher-priority fire, repair, and bridge interaction points; the item-economy smoke then explicitly rebuilds fire-suppression points after its deliberate ignition. This isolates the intended breach-seal target from co-located dispatcher precedence without bypassing production behavior. Bounded read-only spec review `deleg_efb4c713` inspected exactly `73851214` against `3501cbe9` and returned `VERDICT: PASS` without findings, confirming unchanged pass markers, real dispatcher/channel call counts, two isolation blocks, and no direct completion shortcut. `73851214` is an ancestor of `09eb2584`. Fresh parent runs of both affected smokes at current handoff HEAD pass with zero Godot diagnostics. This is a valid first-stage source-spec approval; it is not represented as a separate code-quality verdict.
