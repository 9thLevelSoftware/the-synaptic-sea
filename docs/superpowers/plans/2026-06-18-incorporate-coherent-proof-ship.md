...
- The project path is not expected to be a git repository from the shell; every record step must use a commit-or-record fallback writing to `/tmp/synaptic_sea_coherent_incorporation_no_git_changes.log` if `git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars rev-parse --is-inside-work-tree` fails.
...
Run this shell block from `/Users/christopherwilloughby/the-synaptic-sea-of-stars`:
...
python3 - <<'PY'
from pathlib import Path
proof = Path('/Users/christopherwilloughby/the-synaptic-sea-of-stars/docs/superpowers/proofs/coherent-proof-ship.md')
...
- The no-git ledger is outside the repo at `/tmp/synaptic_sea_coherent_incorporation_no_git_changes.log`; implementers must not create repo-local audit logs.
...
- No-git condition: every record step uses the required commit-or-record fallback and writes to `/tmp/synaptic_sea_coherent_incorporation_no_git_changes.log` when the workspace is not a git repo.
