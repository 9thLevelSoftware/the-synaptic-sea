...
First add a public static to `scripts/systems/dock_ports.gd` (reuses the existing `_room_floor_center` machinery so the bridge center is derived from a layout dict uniformly for ALL ship types):
...
Then the coordinator locator reads the ship's `built_layout` (populated for the lifeboat via `build_layout()`, for derelicts via `get_layout_copy()` on attach, and for the home ship from the loader) — NOT the scene_root loader, because the lifeboat's `scene_root` is a plain `Node3D` with no `get_room_center`:
...
Run: `"$GODOT" --headless --path "$ROOT" --script res://scripts/validation/bridge_terminal_login_smoke.gd`
Expected: `BRIDGE TERMINAL LOGIN SMOKE PASS piloted=lifeboat` (the smoke ends by switching command back to the lifeboat), no unexpected ERROR/WARNING.
