...
- Because this is not a git repository, every task uses the no-git ledger fallback at `/tmp/synaptic_sea_timed_fire_no_git_changes.log` instead of assuming `git commit` works.
...
Run:
```bash
OUT=$(/Users/christopherwilloughby/.local/bin/godot-4.6.2 --headless --path /Users/christopherwilloughby/the-synaptic-sea-of-stars --script res://scripts/validation/fire_state_smoke.gd 2>&1)
printf '%s\n' "$OUT"
...
if git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars add scripts/validation/fire_state_smoke.gd
  git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars commit -m 'test(fire): RED fire model smoke'
else
  printf '%s\n' 'NO_GIT Task 1 RED: scripts/validation/fire_state_smoke.gd added and failed for missing FireState implementation' >> /tmp/synaptic_sea_timed_fire_no_git_changes.log
fi
```
...
```bash
if git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars add scripts/systems/fire_state.gd scripts/validation/fire_state_smoke.gd
  git -C /Users/christopherwilloughby/the-synaptic-sea-of-stars commit -m 'feat(fire): FireState model and model smoke'
else
  printf '%s\n' 'NO_GIT Task 2 GREEN: scripts/systems/fire_state.gd and scripts/validation/fire_state_smoke.gd added and passing' >> /tmp/synaptic_sea_timed_fire_no_git_changes.log
fi
```
...
Run:
```bash
OUT=$(/Users/christopherwilloughby/.local/bin/godot-4.6.2 --headless --path /Users/christopherwilloughby/the-synaptic-sea-of-stars --script res://scripts/validation/main_playable_slice_fire_smoke.gd 2>&1)
printf '%s\n' "$OUT"
...
- `SYNAPTIC_SEA REGRESSION PASS commands=10 clean_output=true` is printed.
...
- `scripts/procgen/playable_generated_ship.gd`
- `docs/game/06_validation_plan.md`
- `/tmp/synaptic_sea_timed_fire_no_git_changes.log` (no-git ledger)
