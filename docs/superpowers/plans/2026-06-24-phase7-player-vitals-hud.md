...
- **Full regression bundle** must end `SYNAPTIC_SEA REGRESSION PASS commands=119 clean_output=true`; stash `project.godot` before the run and pop after (it carries local MCPRuntime-autoload drift that breaks headless).
...
echo 'SYNAPTIC_SEA REGRESSION PASS commands=119 clean_output=true'
```
...
```
Expected final line: `SYNAPTIC_SEA REGRESSION PASS commands=119 clean_output=true`. If `git stash push` reports "No local changes to save" (the drift may already be stashed/clean), proceed and still run the bundle; ensure the `git stash pop` is skipped if nothing was stashed.
