...
**Goal:** Build the deterministic scanner/travel logic — an infinite procgen Synaptic Sea of ship markers, a system/skill-gated scanner, and a propulsion-gated travel action that materializes a real ship via `generate_from_seed`.
...
- The Synaptic Sea grid is the X–Z plane; `position.y` is always `0.0`. Grid cell = `Vector2i(cell_x, cell_y)` where `cell_y` indexes the Z axis.
...
- **Create** `scripts/systems/synaptic_sea_world.gd` — `Synaptic SeaWorld` range query + generated set + summary.
...
- **Create** 4 smokes: `marker_generator_smoke.gd`, `synaptic_sea_world_smoke.gd`, `scanner_state_smoke.gd`, `travel_controller_smoke.gd`.
...
var position: Vector3 = Vector3.ZERO   # y is always 0 (planar Synaptic Sea grid)
...
```bash
"$GODOT" --headless --path "$ROOT" --script res://scripts/validation/synaptic_sea_world_smoke.gd
```
Expected: FAIL — `synaptic_sea_world.gd` does not exist.

- [ ] **Step 3: Implement Synaptic SeaWorld**

Create `scripts/systems/synaptic_sea_world.gd`:

```gdscript
extends RefCounted
class_name Synaptic SeaWorld
...
"$GODOT" --headless --path "$ROOT" --script res://scripts/validation/synaptic_sea_world_smoke.gd
```
Expected: PASS — `SYNAPTIC_SEA WORLD PASS in_range_sorted=true generated=true round_trip=true`.

- [ ] **Step 5: Commit**

```bash
git add scripts/systems/synaptic_sea_world.gd scripts/validation/synaptic_sea_world_smoke.gd
git commit -m "feat(scanner): add Synaptic SeaWorld range query + generated set + summary"
```
...
**Interfaces:**
- Consumes: `Synaptic SeaWorld` (`markers_in_range`, `player_position`).
...
**Interfaces:**
- Consumes: `Synaptic SeaWorld` (`markers_in_range`, `set_player_position`, `mark_generated`, `is_generated`, `player_position`); `ShipMarker`; `ShipGenerator` (`new()`, `generate_from_seed`).
...
In `docs/game/06_validation_plan.md`, inside the `run_clean` block (the bash fence that ends with the `SYNAPTIC_SEA REGRESSION PASS` echo at line ~118), add these four lines just before that echo:
...
run_clean 'synaptic_sea world smoke' 'SYNAPTIC_SEA WORLD PASS in_range_sorted=true generated=true round_trip=true' "$GODOT" --headless --path "$ROOT" --script res://scripts/validation/synaptic_sea_world_smoke.gd
...
Extract the bundle bash fence and run with the Windows paths substituted (do NOT edit the doc's hardcoded macOS paths). The fence content currently spans lines ~30–118; after adding 4 lines it ends ~122 — capture through the `SYNAPTIC_SEA REGRESSION PASS` echo:
```bash
GODOT="C:/Users/dasbl/Documents/Godot/Godot_v4.6.2-stable_win64_console.exe"
ROOT="C:/Users/dasbl/Documents/The Synaptic Sea"
START=$(grep -n '^```bash$' docs/game/06_validation_plan.md | sed -n '2p' | cut -d: -f1)
END=$(grep -n "SYNAPTIC_SEA REGRESSION PASS commands=" docs/game/06_validation_plan.md | head -1 | cut -d: -f1)
...
```
Expected: bundle ends `SYNAPTIC_SEA REGRESSION PASS commands=58 clean_output=true`; Gate-1 prints `GATE 1 AUTOMATED PLAYTEST PASS`. If the count mismatches or an unexpected ERROR/WARNING appears, fix the registration/marker (or the real regression) and re-run — do not adjust the count to mask a missing smoke.
...
- `Synaptic SeaWorld` range query + generated set + summary → Task 2 ✓
