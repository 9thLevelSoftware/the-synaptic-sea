...
```
Then change the success line `echo 'SYNAPTIC_SEA REGRESSION PASS commands=65 clean_output=true'` from `commands=65` to `commands=67`. (Read the actual current count line and increment by 2.)
...
```
Expected final line: `SYNAPTIC_SEA REGRESSION PASS commands=67 clean_output=true`, with no unexpected `ERROR:`/`WARNING:` lines. If any smoke fails or unexpected noise appears, STOP and report the exact line — do not paper over it (no new allowlist entries without a root-cause justification).
