...
- **No `git commit`** assumption — every plan documents the no-git ledger path (`/tmp/sargasso_<feature>_no_git_changes.log`) since `AGENTS.md` flags the workspace as `GIT_INSIDE=false` and the roadmap's own §C confirms it.
...
- **Map screen (U-09)** must hook into existing `SargassoWorld` + `TravelController`; declare the seam explicitly so it doesn't fork a parallel world model. Include a smoke that asserts the map screen refuses to render without scanner detail ≥ 3 (current scanner `detail_level` cap).
...
- 3 plans × ≥3 smokes each = ≥9 new smoke files (`scripts/validation/ui_menu_*.gd`, `ui_hud_*.gd`, `ui_world_*.gd`), each with a unique PASS marker line, all registered in `06_validation_plan.md` and the bundle ends `SARGASSO REGRESSION PASS commands=<N+9> clean_output=true`.
