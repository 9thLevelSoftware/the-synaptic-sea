...
In `docs/game/06_validation_plan.md`, inside the `## Regression bundle` script, add these five lines immediately before the final `echo 'SYNAPTIC_SEA REGRESSION PASS ...'` line:
...
```bash
echo 'SYNAPTIC_SEA REGRESSION PASS commands=47 clean_output=true'
```
...
```
Expected final line: `SYNAPTIC_SEA REGRESSION PASS commands=47 clean_output=true`.
