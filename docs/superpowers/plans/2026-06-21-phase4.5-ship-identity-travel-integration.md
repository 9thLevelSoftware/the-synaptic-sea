...
- `scripts/procgen/playable_generated_ship.gd` — MODIFY. Add `current_ship` wrap at boot; own `synapse_sea_world`/`scanner_state`/`travel_controller`/`ship_generator`; add `scan()`, `travel_to(marker)`, `travel_to_marker_id(id)`; add `away_from_start` gating in `_process`; instantiate + wire the scanner panel; add the `toggle_scanner` input action.
...
This is the core task. It adds the Synapse Sea/scanner/travel ownership, wraps the starting ship, performs the real swap, and gates the starting-ship sim while away.
...
- Produces (relied on by Task 3): on the coordinator —
  ...
  - `func get_current_ship()` and `func get_synapse_sea_world()` validation seams.
...
```
Expected: FAIL — no `TRAVEL INTEGRATION PASS`; first failure is `no PlayableGeneratedShip found`-adjacent OR a runtime error that `get_current_ship`/`scan`/`travel_to_marker_id`/`get_synapse_sea_world`/`scanner_state` are undefined (they do not exist yet).
...
```gdscript
const ShipInstanceScript := preload("res://scripts/systems/ship_instance.gd")
const Synapse SeaWorldScript := preload("res://scripts/systems/synapse_sea_world.gd")
...
var synapse_sea_world         # Synapse SeaWorld
...
- [ ] **Step 4: Build the Synapse Sea/scanner/travel objects at boot**
...
```gdscript
	...
	synapse_sea_world = Synapse SeaWorldScript.new(start_bp.seed_value, Vector3.ZERO)
...
## Phase 4.5 validation seam: the Synapse SeaWorld map model.
...
Wraps the starting ship as current_ship, owns Synapse SeaWorld/ScannerState/
...
In `docs/game/06_validation_plan.md`, after the `travel controller smoke` line (line 121) and before `echo 'SYNAPSE_SEA REGRESSION PASS commands=58 ...'` (line 122), insert:
...
```bash
echo 'SYNAPSE_SEA REGRESSION PASS commands=58 clean_output=true'
```
to:
```bash
echo 'SYNAPSE_SEA REGRESSION PASS commands=61 clean_output=true'
```
...
END=$(grep -n "SYNAPSE_SEA REGRESSION PASS commands=" "$ROOT/docs/game/06_validation_plan.md" | head -1 | cut -d: -f1)
sed -n "$((START+1)),${END}p" "$ROOT/docs/game/06_validation_plan.md" > /tmp/synapse_sea_bundle.sh
GODOT="$GODOT" ROOT="$ROOT" bash /tmp/synapse_sea_bundle.sh
```
Expected: the run ends with `SYNAPSE_SEA REGRESSION PASS commands=61 clean_output=true`. Any `run_clean` failure prints the offending label + first unexpected line — fix the cited smoke before proceeding.

> If the bundle's first `bash` fence is not the runnable bundle, adjust the `sed -n '2p'`/`'1p'` selector. The bundle is the fence that defines `run_clean` and ends with the `SYNAPSE_SEA REGRESSION PASS` echo.
...
- Coordinator owns Synapse SeaWorld/ScannerState/TravelController + `scan()`/`travel_to()` → Task 2. ✓
