...
sed -n '30,160p' "$ROOT/docs/game/06_validation_plan.md" > /tmp/synaptic_sea_bundle.sh
GODOT="$GODOT" ROOT="$ROOT" bash /tmp/synaptic_sea_bundle.sh
git stash pop
```
Expected final line: `SYNAPTIC_SEA REGRESSION PASS commands=88 clean_output=true`. (Adjust the `sed` range if the bundle script block moved after editing the doc.)
