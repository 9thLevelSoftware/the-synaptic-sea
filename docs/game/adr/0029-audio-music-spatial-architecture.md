...
Six pure RefCounted models under `scripts/systems/`, plus the AudioBusConfig Resource. Each follows the standard Sargasso model contract (configure / tick / get_summary / apply_summary / get_status_lines) — the same shape as `FireState` and `ElectricalArcState`, adapted for non-passability audio state:
