...
- **Feature spec `features/hub_progression.md` is NOT authored during Gate 1.** Doing so would imply in-scope work and is treated as a scope violation by `synapse_sea_review`.
...
- Documentation review by `synapse_sea_review` confirms:
