...
- **Feature spec `features/hub_progression.md` is NOT authored during Gate 1.** Doing so would imply in-scope work and is treated as a scope violation by `synaptic_sea_review`.
- **Re-decision trigger:** before Gate 2 begins, a hub/meta re-decision card on board `synaptic-sea-stage-gate` must select exactly one of:
...
- Documentation review by `synaptic_sea_review` confirms:
