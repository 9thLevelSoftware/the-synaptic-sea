...
- Full-bundle marker `SARGASSO REGRESSION PASS commands=163 clean_output=true` after resolving the tracked rigid-pair/adjacent validation preconditions.
...
- Given the full regression bundle in `docs/game/06_validation_plan.md`, when it runs against the Task 14 retry workspace, then it emits `SARGASSO REGRESSION PASS commands=163 clean_output=true` with no unallowlisted `ERROR:`/`WARNING:` lines.
