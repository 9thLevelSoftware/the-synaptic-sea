...
`tests/test_meshy_blender_tools.py`, and the existing fixtures
`tests/fixtures/biomass_valid_core_wrapper.tscn`,
`tests/fixtures/biomass_valid_nested_wrapper.tscn`,
`tests/fixtures/biomass_node_overflow_wrapper.tscn`, and
`tests/fixtures/biomass_forbidden_physics_wrapper.tscn`. The four fixture root metadata mappings are
respectively `biomass_part_id=biomass_humanoid_torso_v1`, `biomass_part_id=biomass_claw_v1`,
`biomass_part_id=biomass_cephalopod_tentacle_v1`, and `biomass_part_id=biomass_maw_v1`;
the two intentionally invalid fixtures retain correct identity metadata. RED tests first prove current
acceptance of mesh-node extras `{"biomass_socket":true}`.
...
preserving the explicit three-argument APIs and closed catalog schema.

The readiness-doc regression bundle now contains 658 actual `run_clean` lines, including the seven Task 9 registrations appended after the pre-existing bundle. The live bundle marker is exactly `SYNAPTIC_SEA REGRESSION PASS commands=658 clean_output=true`. Before execution, mechanically count only lines whose trimmed text starts with `run_clean ` and compare that count to the final live marker's numeric value, for example:
...
# load_denied_sfx_smoke and its away twin deliberately exercise request_load
# with no compatible world save; the denial warning is the behavior under test.
LOAD_DENIED_WARNING="^WARNING: PlayableGeneratedShip: no compatible world save to load\$"
...
run_clean() {
  ...
  printf '%s\n' "$OUT" | grep -q "$marker"
  EXTRA_WARNING=""
  if [ "$label" = 'Load denied SFX smoke' ] || [ "$label" = 'Load denied away smoke' ]; then
    EXTRA_WARNING="|$LOAD_DENIED_WARNING"
  fi
...
# seed_000017 authority is the current production generator (MEDIUM/PRISTINE
# seed 17): 10 rooms, 29 portals, 69 placements, 51 floors, 4 objectives.
# Fixtures are JSON-safe arrays with structural_plan.validated=true; do not
# restore the historical 84/51 pair. Marker is the observed loader line.
run_clean 'procgen loader playable contract smoke' 'PROCGEN LOADER PLAYABLE CONTRACT PASS loaded=true objectives=4 collision_shapes=251 structural_live=true edge_wrappers=69 floor_wrappers=51' "$GODOT" --headless --path "$ROOT" --script res://scripts/validation/procgen_loader_playable_contract_smoke.gd
...
# Task 9 registrations (append after Task 8 evidence)
...
run_clean 'Biomass assembly smoke' 'BIOMASS GAIT PASS recipes=5 profiles=5 deterministic=true bounded=true rest=true drift=false' "$GODOT" --headless --path "$ROOT" --script res://scripts/validation/biomass_assembly_smoke.gd
...
run_clean 'Biomass composite review verify' 'BIOMASS COMPOSITE REVIEW PASS stage=placeholder gaits=5 seeds=2 lighting=3 captures=30' /opt/homebrew/bin/python3.11 tools/biomass_composite_review.py verify --project-root . --report artifacts/validation-previews/biomass-assembly-placeholder/review.json
...
- [x] Derelict fire — 5 smokes (M7-B / ADR-0041 / this branch `feat/derelict-fire`):
  ...
  - `scripts/validation/ship_instance_fire_persistence_smoke.gd` (marker `SHIP INSTANCE FIRE PERSISTENCE PASS omitted=true restored=true seeded_vents=true`) — proves the "fire" key is omitted from `ShipInstance.get_summary()` when the ship was never seeded and nothing burns/vents (no snapshot bloat), that a burning state is faithfully restored via `apply_summary()`, and that a fire_seeded vents-only model still persists so load does not drop `vented_compartments`.
