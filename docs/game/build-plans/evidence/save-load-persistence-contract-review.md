...
Author: synaptic_seaworker (Task 11, board `synaptic-sea-e2e-systems`)
Date: 2026-06-25
...
| Path | Role |
|---|---|
| `scripts/systems/save_slot_state.gd` | Pure data class representing one row in the save index. `slot_id`, `slot_kind` ("manual"/"auto"/"quick"/"world"), `display_name`, `synaptic_sea_seed`, `player_class`, `current_location`, `objective_sequence`, `play_time_seconds`, `saved_at`, `world_slot_id` (for embedded ship slots inside a world), `corrupt` (bool). |
