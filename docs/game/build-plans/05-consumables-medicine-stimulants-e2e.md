# Task 05: Consumables, Medicine, Stimulants, Ammo & Utility Items — Standalone E2E Package Plan

Generated: 2026-06-25T19:59:51-04:00

## Objective
Deliver a complete, production-grade, independently testable package for **Consumables, Medicine, Stimulants, Ammo & Utility Items**.

## Source requirements
- Package requirement range: REQ-CN-001..010
...
## Concept lock
Single consumable/effect pipeline covering medicine, stims, repair kits, ammo, utility items, trade goods, addiction/withdrawal, status cures, and hotbar use.

## Project Zomboid influence, translated in-character
PZ first-aid, bandages, painkillers, beta blockers, infection treatment, weapon ammo scarcity; expanded with sci-fi stimulant dependency.
...
- medicine/stim/ammo/utility/trade item definitions
- addiction tuning
- status cure tables
...
- hotbar use action
- stim withdrawal tick
- medicine/status cure integration
- ammo consumption integration

### Required UI/UX/player feedback
- Use/Use All context menu
- hotbar consumable slots
- status effect icons
- withdrawal warnings

### Required asset/audio seams
- medicine/stim/ammo/utility icons
- status effect icons

### Required smoke/validation files
- effect_dispatcher_smoke.gd
- consumable_state_smoke.gd
- medicine_state_smoke.gd
- stimulant_state_smoke.gd
- addiction_state_smoke.gd
- main_playable_slice_consumables_smoke.gd
- consumable_save_load_smoke.gd

### Required PASS markers
- EFFECT DISPATCHER PASS
- CONSUMABLE STATE PASS
- STIMULANT STATE PASS
- MAIN PLAYABLE CONSUMABLES PASS

## Acceptance criteria
- Every consumable uses the shared effect dispatcher; no per-item special-case branch islands.
- Medicine heals/cures defined statuses.
- Stimulants apply real timed buffs, tolerance, addiction, and withdrawal.
- Buff/withdrawal state persists across save/load.
...
- Write a short `docs/game/build-plans/evidence/consumables-medicine-stimulants-contract-review.md` with existing files, missing files, and chosen extension seams.
...
- Add requirement rows for REQ-CN-001..010 with status transitions: Proposed -> Approved -> Validated.
