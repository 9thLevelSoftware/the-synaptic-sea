# Task 15: Systems Map, Roadmap, Requirements, Manifest & Final Board Currency — Standalone E2E Package Plan

Generated: 2026-06-25T19:59:51-04:00

## Objective
Deliver a complete, production-grade, independently testable package for **Systems Map, Roadmap, Requirements, Manifest & Final Board Currency**.

## Source requirements
- Package requirement range: REQ-DOC-001..008
...
## Concept lock
Final source-of-truth update: systems map, roadmap, requirements, ADR index, validation plan, task graph manifest, build plan, and dashboard/board currency checks.

## Project Zomboid influence, translated in-character
Not a gameplay mechanic; this preserves production discipline so future autonomous agents do not rebuild stale or partial systems.
...
- SystemsMapCurrencyValidator
- RequirementTraceValidator
- AdrIndexValidator
- KanbanManifestValidator
...
- doc validators and kanban stats checks
...
- systems_map_currency_smoke.gd or docs validator script
- requirement_trace_smoke.gd
- kanban_manifest_smoke.py
...
## Acceptance criteria
- Every completed package has source-backed file evidence in the systems map.
- Manifest uses real Kanban IDs and matches board task/link counts.
- Requirements and ADR index reference shipped artifacts.
- No stale deferral language remains for features that are in scope.
...
- Write a short `docs/game/build-plans/evidence/systems-map-task-graph-update-contract-review.md` with existing files, missing files, and chosen extension seams.
...
- Add requirement rows for REQ-DOC-001..008 with status transitions: Proposed -> Approved -> Validated.
